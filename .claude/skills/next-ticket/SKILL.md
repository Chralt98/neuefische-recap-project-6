...
3. Read the ticket's current `stage:*` label and invoke exactly the matching skill via the Skill tool:
...
4. Do not chain into the next stage automatically — stop after the invoked skill returns so the user can review its output, per the pipeline's "review and validate each step" rule.
5. Report which ticket and stage ran, and what the next stage will be once this one is approved.

## Goal
Keep exactly one ticket moving through the pipeline at a time, with a human checkpoint between every stage.
